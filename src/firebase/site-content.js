...
export async function getSiteContent() {
    console.log('[SiteContent] === FETCHING SITE CONTENT ===');
    try {
        // Check cache first
        const cached = getCachedContent();
        if (cached) {
            console.log('[SiteContent] ✅ Using cached content');
            console.log('[SiteContent] Hero title:', cached.hero?.title);
            return cached;
        }

        console.log('[SiteContent] 📡 Fetching from Firestore (siteContent/main)...');
        ...
        if (docSnap.exists()) {
            ...
            console.log('[SiteContent] Hero title:', content.hero?.title);
            console.log('[SiteContent] Full content:', content);
            ...
            console.warn('[SiteContent] ⚠️ No Firebase document found, using defaults');
            const defaults = getDefaultContent();
            console.log('[SiteContent] Default hero title:', defaults.hero?.title);
            return defaults;
        }
    } catch (error) {
        console.error('[SiteContent] ❌ Error fetching content:', error);
        const defaults = getDefaultContent();
        console.log('[SiteContent] Fallback to defaults, hero title:', defaults.hero?.title);
        return defaults;
    }
}

/**
 * Clear the site content cache
 */
export function clearSiteContentCache() {
    try {
        sessionStorage.removeItem(CACHE_KEY);
        console.log('[SiteContent] Cache cleared');
        return true;
    } catch (error) {
        console.error('[SiteContent] Failed to clear cache:', error);
        return false;
    }
}
