...
function initEnhancedStatsCounter() {
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                const element = entry.target;
                // Only animate if we have a valid target > 0
                const target = parseInt(element.getAttribute('data-target'));
                if (target > 0 && !element.classList.contains('animated')) {
    ...
    // Listen for dynamic updates from content-loader.js
    window.addEventListener('statsUpdated', () => {
        stats.forEach(stat => {
            stat.classList.remove('animated'); // Reset animation state
