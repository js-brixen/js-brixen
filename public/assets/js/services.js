...
(function () {
    ...
    function createCardHTML(service) {
        ...
        const bookingURL = `/book-consultation.html?service=${service.slug}&serviceTitle=${encodeURIComponent(service.title)}`;
        const detailURL = `/service-template.html?service=${service.slug}`;

        return `
      <article class="service-card" tabindex="0" role="link" 
        ...
        <div class="service-card__content">
          <h2 id="service-title-${service.slug}" class="service-card__title">${service.title}</h2>
          <p class="service-card__description">${service.shortDescription}</p>
          
          <div class="service-card__actions">
            <a href="${bookingURL}" 
               class="btn-primary btn-sm"
               onclick="trackCTAClick('${service.slug}', 'book_consultation'); event.stopPropagation();">
              Book Consultation
            </a>
            <a href="${detailURL}" 
    ...
    function populateServicePage(service) {
        ...
        if (heroSummary) heroSummary.textContent = service.fullDescription;
      ...
      `).join('');
        ...
        if (processTimeline && service.process) {
            processTimeline.innerHTML = service.process.map(step => `
        <li class="process-step" data-step="${step.step}">
      ...
      `).join('');
        ...
        if (accordion && service.faqs) {
            accordion.innerHTML = service.faqs.map((faq, index) => `
        <div class="accordion-item">
          <h3>
            <button class="accordion-trigger" aria-expanded="false" aria-controls="faq-${index}">
              <span>${faq.question}</span>
              <span class="accordion-icon" aria-hidden="true">+</span>
            </button>
          </h3>
          <div id="faq-${index}" class="accordion-panel" hidden>
            <p>${faq.answer}</p>
          </div>
        </div>
      `).join('');
